...
def carregar_dados_clickhouse():
    """Carrega dados do ClickHouse com cache."""
    try:
        ...
        st.warning(f"Erro ao conectar ao ClickHouse: {e}. Usando dados sintéticos.")
        return load_data('synthetic')

@st.cache_data(ttl=60)  # Cache por 1 minuto
def carregar_scores_clickhouse():
    """Carrega scores calculados do ClickHouse."""
    try:
        from clickhouse_client import ClickHouseClient
        client = ClickHouseClient()
        return client.get_detailed_scores()
    except Exception as e:
        st.warning(f"Erro ao carregar scores: {e}")
        return None

@st.cache_data(ttl=60)
def carregar_top_numeros(n=20):
    """Carrega top N números do ClickHouse."""
    try:
        from clickhouse_client import ClickHouseClient
        client = ClickHouseClient()
        return client.get_top_numbers(n)
    except Exception as e:
        return None

@st.cache_data(ttl=60)
def carregar_faltantes_ciclo():
    """Carrega números faltantes no ciclo."""
    try:
        from clickhouse_client import ClickHouseClient
        client = ClickHouseClient()
        return client.get_numeros_faltantes_ciclo()
    except Exception as e:
        return []
...
def main():
    ...
    if 'dados_carregados' not in st.session_state:
        ...
        # Tentar carregar do ClickHouse primeiro
        try:
            with status_placeholder.container():
                st.info("🔗 **Conectando ao ClickHouse...**")

            df_clickhouse = carregar_dados_clickhouse()

            if df_clickhouse is not None and len(df_clickhouse) > 0:
                st.session_state.dados_carregados = df_clickhouse
                st.session_state.nome_arquivo = "ClickHouse Database"
                st.session_state.fonte_dados = "ClickHouse (Real-time)"
                st.session_state.caminho_arquivo = "localhost:9000/loterias.megasena"
                st.session_state.usando_clickhouse = True

                status_placeholder.success(f"✅ **Conectado ao ClickHouse:** {len(df_clickhouse):,} concursos")
            else:
                raise Exception("Dados vazios do ClickHouse")

        except Exception as e:
            # Fallback para arquivo Excel
            ...
            if os.path.exists(ARQUIVO_PADRAO):
                try:
                    with status_placeholder.container():
                        st.warning(f"⚠️ ClickHouse indisponível. Carregando do arquivo Excel...")
                    ...
                    if colunas_dezenas:
                        ...
                        st.session_state.fonte_dados = "Arquivo Excel"
                        st.session_state.caminho_arquivo = ARQUIVO_PADRAO
                        st.session_state.usando_clickhouse = False

                        status_placeholder.success(f"✅ **Carregado:** {len(df_padrao):,} concursos do arquivo")
                    else:
                        raise Exception("Formato não reconhecido")
                except Exception as e2:
                    st.session_state.dados_carregados = carregar_dados_sinteticos()
                    st.session_state.nome_arquivo = None
                    st.session_state.fonte_dados = "Demonstração"
                    st.session_state.caminho_arquivo = None
                    st.session_state.usando_clickhouse = False
                    status_placeholder.warning(f"⚠️ Usando dados de demonstração: {str(e2)}")
            else:
                st.session_state.dados_carregados = carregar_dados_sinteticos()
                st.session_state.nome_arquivo = None
                st.session_state.fonte_dados = "Demonstração"
    ...
    st.markdown('<div class="sub-header">Análise Estatística Avançada com 13 Critérios</div>', unsafe_allow_html=True)
